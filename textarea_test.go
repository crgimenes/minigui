...
func TestCaretUpDownKeepsColumn(t *testing.T) {
	r := []rune("ab\ncd")
	got := caretDown(r, 1)
	if got != 4 { // line0 col1 -> line1 col1
...
func TestCaretDownClampsColumn(t *testing.T) {
	...
	r := []rune("hello\nhi")
	got := caretDown(r, 4)
	if got != 8 { // line1 has len 2 -> end at index 8
...
func TestEditTextAreaHomeEndLineRelative(t *testing.T) {
	_, caret, _ := editTextArea("ab\ncd", 4, 4, Input{Home: true})
	if caret != 3 {
		t.Errorf("home on line 2: caret %d, want 3", caret)
	}
	_, caret, _ = editTextArea("ab\ncd", 3, 3, Input{End: true})
	if caret != 5 {
